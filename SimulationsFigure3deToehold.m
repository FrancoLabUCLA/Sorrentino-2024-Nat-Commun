...
close all;
HighYield=1;
LowYield=0; % OPTION TO RUN THIS SIMULATION BUT NOT DONE FOR THE PAPER 
Parameters % LOADS PARAMETERS
nM=1e9;
hours=3600; % CONVERSION FACTOR FROM SECONDS TO HOURS
SimulationTime=10; % HOURS
...
p.kRSnominal = 4e4;% 6nt
Array=[0.002 0.03 0.7 1];
Toehold=[3 4 5 6];
epsilon=1;
...
filename = 'SimulationsFig3de.xlsx';
Delay=InitT2-InitT1;
...
subplot(1,2,1)
plot(Toehold,Delay,'o:k','MarkerSize',MSize,'MarkerFaceColor','k')
xlim([Toehold(1)-1 Toehold(end)+1])
ylim([0 120])
title('\Delta T (min)')
xlabel('Toehold length')
subplot(1,2,2)
plot(Toehold,HalfT1,'o:','Color',LGreen,'MarkerSize',MSize,'MarkerFaceColor',LGreen)
hold on
plot(Toehold,HalfT2,'o:','Color', DRed,'MarkerSize',MSize,'MarkerFaceColor',DRed)
xlim([Toehold(1)-1 Toehold(end)+1])
ylim([0 250])
title('t_{1/2}  (min)')
xlabel('Toehold length')
...
if HighYield==1
    ...
    print(gcf,'-dpdf', 'Figure3de_HighYield.pdf') % PRINTS TO A FILE.
 else if LowYield==1
        figure(1)
        set(gcf, 'PaperUnits', 'centimeters'); % SETS THE PAPER UNITS
        set(gcf, 'PaperPosition', [0 0 Width Height]); % SETS THE FIGURE SIZE
        set(gcf, 'PaperSize', [Width Height]); % CUTS THE FIGURE
        print(gcf,'-dpdf', 'Figure3de_LowYield.pdf') % PRINTS TO A FILE.
     end
end
